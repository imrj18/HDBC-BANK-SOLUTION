...
public class TransactionController {
    ...
    public ResponseEntity<List<TransactionHistoryDTO>> history(@AuthenticationPrincipal CustomerPrincipal principal,
                                                               @Valid @RequestParam(required = false) Long accountNum){
        String email = principal.getUsername();
        return ResponseEntity.ok(transactionService.transactionHistory(email,accountNum));
    }
