...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDuplicate(CustomerAlreadyExistsException ex) {
        log.warn("Customer already eixsts : {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.CONFLICT).body(new ErrorResponse(ex.getMessage(), 403));
    }
    ...
    public ResponseEntity<ErrorResponse> handleValidationErrors(WrongPasswordException ex) {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ErrorResponse(ex.getMessage(), 403));
    }
